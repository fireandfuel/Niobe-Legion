				...
				if(this.reader != null)
				{
					...
					switch(this.reader.getEventType())
					{
						case START_ELEMENT:
							...
							Logger.debug(LegionLogger.RECEIVED, "received START_ELEMENT : "
									+ this.currentStanza.getName());

							for(int i = 0; i < this.reader.getAttributeCount(); i++)
							{
								this.currentStanza.getAttributes().put(
							...
							{
								if(Communicator.LEGION_NAMESPACE_URI.equals(this.currentStanza
										.getNameSpaceURI()))
								{
									this.consumeStartElement(this.currentStanza);
								} else
								{
									ICommunicator communicator = Communicator.MODULE_COMMUNICATORS
						...
						case CHARACTERS:
							...
							Logger.debug(LegionLogger.RECEIVED,
									"received CHARACTERS : " + this.currentStanza.getName()
						...
						case END_ELEMENT:
							...
							Logger.debug(LegionLogger.RECEIVED, "received END_ELEMENT : "
									+ this.currentStanza.getName());
						...
						default:
							Logger.debug(LegionLogger.RECEIVED,
									"Unknown event type " + this.reader.getEventType());
							break;
					}
				} else
				{
			...
			switch(message.getEventType())
			{
				...
				case CHARACTERS:
					...
					} else
					{
						for(String attrName : message.getAttributes().keySet())
						{
							this.out.write((" " + attrName + "=\""
									+ message.getAttributes().get(attrName) + "\"")
									.getBytes("UTF-8"));
						}
						this.out.write(">".getBytes("UTF-8"));
					}
