...
final class ScramClient extends ScramBase implements SaslClient
{
	...
	public ScramClient(String hashAlgorithm, String authorizationId, String protocol,
			...
			throws SaslException
	{
		...
		this.gs2Header = ScramClient.GS2_CBIND_FLAG + ","
				+ (authorizationId != null ? "a=" + authorizationId : "") + ",";
	}
	...
	public byte[] evaluateChallenge(byte[] challenge) throws SaslException
	{
		...
		if(challenge.length == 0)
		{
			...
			} catch (UnsupportedCallbackException e)
			{
				throw new SaslException(
						"SCRAM: Cannot perform callback to acquire username or password", e);
			}
			...
			this.clientFirstMessageBare = ScramBase.createClientFirstMessageBare(this.username,
					this.cnonce);
		...
		} else
		{
			...
			if(attributes.containsKey('v'))
			{
				...
				try
				{
					byte[] saltedPassword = this.computeSaltedPassword(this.passwd, this.salt,
							this.iterationCount);
					byte[] serverKey = this.computeServerKey(saltedPassword);
					byte[] calculatedServerSignature = this.computeServerSignature(serverKey,
							this.computeAuthMessage());
					String serverSignatureBase64 = attributes.get('v');

					if(serverSignatureBase64 == null)
					{
						throw new SaslException(
								"SCRAM: server signature was null in the server response.");
					}
			...
			} else
			{
				...
				{
					throw new SaslException(
							"SCRAM: iterationCount was null in the server response.");
				}
				...
				try
				{
					this.channelBinding = DatatypeConverter.printBase64Binary(this.gs2Header
							.getBytes());
					byte[] clientKey = this.computeClientKey(this.computeSaltedPassword(
							this.passwd, this.salt, this.iterationCount));
					byte[] clientSignature = this.computeClientSignature(clientKey,
							this.computeAuthMessage());
					// ClientProof := ClientKey XOR ClientSignature
					byte[] clientProof = ScramBase.xor(clientKey, clientSignature);
					this.clientFinalMessageWithoutProof = "c=" + this.channelBinding + ",r="
							+ this.nonce;
