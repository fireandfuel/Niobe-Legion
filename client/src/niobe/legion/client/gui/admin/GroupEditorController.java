...
public class GroupEditorController implements ICloseableDialogController, DatasetReceiver<GroupRightEntity>
{
    ...
    private final EventHandler<KeyEvent> keyHandler = (event) -> GroupEditorController.this.saveButton
            .setDisable(GroupEditorController.this.groupName == null || GroupEditorController.this.groupName.getText()
                    .length() < 2);
    ...
    private TreeItem<FxDatasetWrapper<GroupRightEntity>> createRightNodes(IRight[] rights)
    {
        if(rights != null)
        {
            List<TreeItem<FxDatasetWrapper<GroupRightEntity>>> groupRightEntities = Stream.of(rights)
                    .filter(IRight::isRoot).map(this::createRightNode).collect(Collectors.toList());
