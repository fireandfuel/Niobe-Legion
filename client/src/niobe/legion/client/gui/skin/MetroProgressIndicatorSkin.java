		...
		// BehaviorBase<ProgressIndicator>>
{
	...
	private class MetroIndetermineSpinner extends Region
	{
		...
		{
			return (Math.abs(Math.sin((x * Math.PI) / 360.0d) * MetroProgressIndicatorSkin.this.maxStepTime.get()));
		}
		...
		private void rebuildTimeline()
		{
			if (this.spinEnabled)
			{
				if (MetroProgressIndicatorSkin.this.indeterminateTransition == null)
				{
					...
					MetroProgressIndicatorSkin.this.indeterminateTransition.setCycleCount(Animation.INDEFINITE);
				...
				} else
				{
					...
					((Timeline) MetroProgressIndicatorSkin.this.indeterminateTransition).getKeyFrames().clear();
				}
				...
				for (int c = 0; c < MetroProgressIndicatorSkin.this.indeterminateSegmentCount.get(); c++)
				{
					keyFrames.add(new KeyFrame(Duration.millis(1),
											   new KeyValue(this.pathsG.getChildren().get(c).rotateProperty(), 0),
											   new KeyValue(this.pathsG.getChildren().get(c).opacityProperty(), 0)));

					double time = MetroProgressIndicatorSkin.this.nextSegmentTime.get() * c;
					for (int i = 0; i <= 720; i += MetroProgressIndicatorSkin.this.anglePerStep.get())
					{
						time += this.timePerAngle(i);

						keyFrames.add(new KeyFrame(Duration.millis(time),
												   new KeyValue(this.pathsG.getChildren().get(c).rotateProperty(), i),
												   new KeyValue(this.pathsG.getChildren().get(c).opacityProperty(),
																this.opacityPerAngle(i))));
					}
					keyFrames.add(new KeyFrame(Duration.millis(
							time + MetroProgressIndicatorSkin.this.nextSpinTime.get()),
											   new KeyValue(this.pathsG.getChildren().get(c).rotateProperty(), 0),
											   new KeyValue(this.pathsG.getChildren().get(c).opacityProperty(), 0)));
				}

				((Timeline) MetroProgressIndicatorSkin.this.indeterminateTransition).getKeyFrames().setAll(keyFrames);
				MetroProgressIndicatorSkin.this.indeterminateTransition.playFromStart();
			} else
			{
				if (MetroProgressIndicatorSkin.this.indeterminateTransition != null)
		...
		protected void layoutChildren()
		{
			...
			this.pathsG.resizeRelocate((w - indicatorW) / 2, (h - indicatorH) / 2, indicatorW, indicatorH);
		}
	...
	protected void initialize()
	{
		if (this.control != null)
		{
			...
			{
				this.spinner = new MetroIndetermineSpinner(this.spinEnabled.get(), this.progressColor.get());
	...
	private static final CssMetaData<ProgressIndicator, Paint> PROGRESS_COLOR =
			new CssMetaData<ProgressIndicator, Paint>("-fx-progress-color", PaintConverter.getInstance(), null)
			{
	...
	private static final CssMetaData<ProgressIndicator, Number> ANGLE_PER_STEP =
			new CssMetaData<ProgressIndicator, Number>("-fx-indeterminate-angle-per-step",
													   SizeConverter.getInstance(),
													   40)
			{
	...
	private static final CssMetaData<ProgressIndicator, Number> MAX_STEP_TIME =
			new CssMetaData<ProgressIndicator, Number>("-fx-indeterminate-max-step-time",
													   SizeConverter.getInstance(),
													   300.0d)
			{
	...
	private static final CssMetaData<ProgressIndicator, Number> NEXT_SEGMENT_TIME =
			new CssMetaData<ProgressIndicator, Number>("-fx-indeterminate-next-segment-time",
													   SizeConverter.getInstance(),
													   200.0d)
			{
	...
	private static final CssMetaData<ProgressIndicator, Number> NEXT_SPIN_TIME =
			new CssMetaData<ProgressIndicator, Number>("-fx-indeterminate-next-spin-time",
													   SizeConverter.getInstance(),
													   500.0d)
			{
